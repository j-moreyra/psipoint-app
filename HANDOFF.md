...
- Deployed to Netlify production at https://psipoint.app (cutover 2026-05-10; see § 5).
...
- **Hand-rolled service worker over Workbox / `next-pwa`.** MVP needs app-shell cache + install prompt — not route-level strategies, background sync, or broadcast channels. `public/sw.js` is ~90 lines of cache-first + network-first. If offline sync lands in v2, swap to Workbox then. Adding a dep for what one file covers would have been speculative design.
- **Dashboard-only license expiration banner — not app header.** Putting a warn-tone banner above every screen is the kind of nag that gets ignored within a week. Dashboard is the first landed surface; that's the one place the copy can breathe. Header real-estate is already full (logo, nav, search, avatar menu).
- **Inline warning for due-date method change, not a confirmation modal.** The app has no `alert-dialog` primitive in `src/components/ui/`, and adding one for a single confirmation was overkill. The amber `SoftNotice` pattern from the test form (gauge-change, stale-cal hints) covers the case with the same visual grammar. User acknowledges by clicking the now-enabled Save button.
- **No backfill when `next_due_calculation_method` changes.** Rewriting every device's `next_test_due_date` when the rule flips is a data-mutation feature that deserves its own design pass (confirmation, progress, rollback, per-device overrides). Blueprint doesn't call for it. Inline notice explains the implication; existing devices keep their dates until their next test lands.
- **Due-date method is its own card with its own save, not a field on the company form.** Splitting the saves means `companySchema` can drop the field (and its `onSubmit` stays single-responsibility), and the amber "this change applies only to new tests" notice is only reachable when the user is consciously editing the rule — not accidentally scrolling past it while updating the ZIP.
- **localStorage, not IndexedDB.** Test forms are <1KB of JSON; IDB adds async boilerplate for no storage benefit. No cross-tab sync (last write per tab wins) — two tabs on the same device form is a rare case, and the simpler model makes the hook testable.
- **Keyed per-device draft, not per-session.** `backflo:draft:test:v1:<deviceId>` — returning to the same device resumes; different devices don't collide. Version suffix lets a future test-form field-shape change invalidate old drafts silently on read.
- **7-day TTL on drafts.** Covers "started a test last Tuesday, coming back today" without inviting "started a test six months ago, just found out." Dropped silently on read — no UI friction.
- **Test-form draft only this phase.** Customer/location/device/settings forms are short and abandonment isn't a documented pain point. Test entry is the sub-2-minute field-of-use form where losing progress to a phone sleep is the real cost.
- **Placeholder icon shipped, flagged as swap-in.** A real brand mark is the user's call; shipping nothing blocks install verification on the mobile QA pass. `public/icon.svg` source + `npm run build-icons` script make the swap a one-file edit.
- **SW skipped in dev by default.** Dev hot-reloads and SW caching fight each other; SW's whole job is prod install + offline launch. `NEXT_PUBLIC_PWA_DEV=1` opt-in exists for debugging the manifest / icon wiring against `next start`.
- **SW bypasses `/auth/*` and `/api/*`.** Auth cookies + Supabase session refresh must always hit the network. Static assets under `/_next/static/` are immutable per Next's build manifest — cache-first with no revalidation is safe; bust is handled by URL change on deploy.
- **No pgTAP / Playwright this phase either.** Still a priority; still deferred. Phase 5 was blueprint scope; adding a new test harness would have blown the window. Tracked in the testing backlog.
...
**No blocking issues.** Build is clean, tests pass (572), production live at https://psipoint.app, end-to-end certificate generation + email send verified live (cutover 2026-05-10; see § 5).
...
### Lessons learned from Phase 5 shipping

- **`loading.tsx` cascades — inspect before you add.** Next.js 16 applies the nearest-ancestor `loading.tsx` to any descendant segment that doesn't have its own. Phase 2 had shipped `customers/loading.tsx` (list shape) + `customers/[id]/loading.tsx` (detail shape). That meant `/tests/new`, the canonical test form, the certificate page, and everything under `/settings/*` were using the wrong cascade source — enough to look broken on slow navs even though the server render was fine. The fix was five new scoped `loading.tsx` files, not a refactor of the existing ones.
- **Inline notice beats modal when the primitive doesn't exist.** The plan called for a confirmation modal on due-date method change. `src/components/ui/` has no `alert-dialog`; adding one via `npx shadcn@latest add` would have pulled in base-ui dependencies for a single confirmation dialog. The project's existing `SoftNotice` pattern (amber callout inline) covered the same acknowledgment need at a fraction of the surface area. Matched the app's tone, too.
- **`sharp` is a transitive dep via `next 16`.** I expected to add `sharp` as a devDep for the icon raster script; `npm ls sharp` showed it's already pulled in by Next's image-optimization pipeline. One fewer explicit install.
- **Maskable PWA icons need a 10–20% safe zone.** Android launchers crop around the edges to match whatever mask shape they're rendering (circle, squircle, teardrop). Shipping an edge-to-edge logo means the outer strokes get chopped. `scripts/build-icons.ts` renders the BF monogram inside an 80% inner viewport with the brand color as the safe-zone background so the crop falls on solid color.
- **Next metadata emits `mobile-web-app-capable`, not only the legacy apple variant.** `appleWebApp: { capable: true }` in the metadata object surfaces as `<meta name="mobile-web-app-capable" content="yes">` — the modern spec name, honored by current iOS + Android. The legacy `apple-mobile-web-app-capable` isn't emitted; that's intentional and correct.
- **Service worker registration has to be gated on `NODE_ENV === 'production'` or the file has to be served statically.** Registering the SW in dev means turbopack hot-reloads fight the cached app shell, and a page refresh can serve a route from a stale cache while the source file already moved on. `NEXT_PUBLIC_PWA_DEV=1` is the opt-in for debugging against `next start`.
- **react-hook-form's `watch()` subscription is the right hook for draft persistence, not a `useEffect` on values.** `form.watch((values) => ...)` returns a subscription handle whose `.unsubscribe()` cleans up on unmount — documented pattern, doesn't double-fire on re-renders. A naïve `useEffect(() => localStorage.setItem(key, JSON.stringify(getValues())), [getValues])` looks right but fires on every render.
- **Don't write drafts before hydration completes.** The debounced write effect has to check a `hydratedRef` before the first `setItem`, or the mount-time `reset(defaultValues)` phase clobbers a fresh draft with empty strings. Classic race; cheap to fix once spotted. Guarded by `if (!hydratedRef.current) return;` inside the watch callback.
...
- **Resend:** account provisioned; `psipoint.app` domain verified (cutover 2026-05-10). Production keys set as Netlify env vars (`RESEND_API_KEY` + `RESEND_FROM_EMAIL=certificates@psipoint.app`). `.env.local.example` keeps both as blank stubs — local dev doesn't send mail.
